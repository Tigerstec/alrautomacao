...
function renderBudgets(budgets) { 
    ...
    tbody.innerHTML = budgets.map(b => ` 
        <tr class="border-b hover:bg-gray-50"> 
            <td class="py-3 px-4">${b.cliente}</td> 
            <td class="py-3 px-4">${b.email}</td> 
            <td class="py-3 px-4">R$ ${parseFloat(b.valor).toFixed(2)}</td> 
            <td class="py-3 px-4"><span class="px-2 py-1 text-xs rounded-full ${b.status === 'Aprovado' ? 'bg-green-100 text-green-800' : b.status === 'Rejeitado' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}">${b.status}</span></td> 
            <td class="py-3 px-4"> 
                <button onclick='editBudget(${JSON.stringify(b)})' class="text-blue-600 hover:text-blue-800 mr-2">Editar</button> 
                <button onclick="deleteBudget(${b.id})" class="text-red-600 hover:text-red-800">Excluir</button> 
            </td> 
        </tr>`).join(''); 
}
...
function renderAppointments(appointments) { 
    ...
    tbody.innerHTML = appointments.map(a => ` 
        <tr class="border-b hover:bg-gray-50"> 
            <td class="py-3 px-4">${a.cliente}</td> 
            <td class="py-3 px-4">${a.serviceName}</td> 
            <td class="py-3 px-4">${new Date(a.data_agendamento + 'T00:00:00').toLocaleDateString('pt-BR')} ${a.hora_agendamento.substring(0, 5)}</td> 
            <td class="py-3 px-4"><span class="px-2 py-1 text-xs rounded-full ${getStatusColor(a.status)}">${a.status}</span></td> 
            <td class="py-3 px-4"> 
                <button onclick='editAppointment(${JSON.stringify(a)})' class="text-blue-600 hover:text-blue-800 mr-2">Editar</button> 
                <button onclick="deleteAppointment(${a.id})" class="text-red-600 hover:text-red-800">Excluir</button> 
            </td> 
        </tr>`).join(''); 
}
